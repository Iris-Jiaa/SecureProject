...
?>
        ...
        try {
            ...
            if (!in_array('secureappdev', $existingDatabases)) {
				...
				$hashedAdmin = password_hash('AdminPass1!', PASSWORD_DEFAULT);
				$hashedUser1 = password_hash('Password1!', PASSWORD_DEFAULT);

				$makeAdmin = "INSERT INTO `sapusers` (`user_uid`, `user_pwd`, `user_admin`) VALUES ('admin', '$hashedAdmin', '1')";
				$conn->exec($makeAdmin);
				echo "Admin Added (Username = admin, Password =AdminPass1!<br>";
				
				$makeUser1 = "INSERT INTO `sapusers` (`user_uid`, `user_pwd`, `user_admin`) VALUES ('user1', '$hashedUser1', '0')";
				$conn->exec($makeUser1);
        ...
        $conn = null; // Close the database connection
					...
					{
						echo htmlspecialchars($_SESSION['failedMsg'], ENT_QUOTES, 'UTF-8');
